...
- **One case is disclosed, not silently shipped** — a document project whose corpus lives mostly in the four seed filenames (`CHANGELOG.md`, `SESSION_NOTES.md`, `HANDOFFS.md`, `ROADMAP.md`) can still read as `code` after installation, because `bin/sync` leaves those alone when you already have your own and the scanner cannot yet tell yours from a seed it wrote. Not a regression — the previous version behaves identically — and declaring `doc-only` in `.methodology-profile` fixes it today.
