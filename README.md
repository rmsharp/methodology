...
| `FRAMEWORK_LEARNINGS.md` | The framework's own accumulated learnings — reference, read on demand, synced read-only |
...
**Why the 2,000-line cap is a guard worth having, and what it actually is.** It is a **line-denominated proxy**: the agent read cap is denominated in **tokens**, so no line count converts to it faithfully — measured across real markdown the spread is 2.42–2.66 B/token, enough that publishing a conversion would just be a new wrong constant. Past the real cap a read returns **only the prefix that fits**, and it **says so**: a `PARTIAL view` banner names the delivered span, the file's true length, the token count and the cap, and warns against answering from that page; an explicit over-cap line range errors outright and returns nothing at all. So the risk is not a silent read — it is **Phase 0 reconciling from a tail it was told it did not get**. An earlier version of this section asserted the opposite — that the truncation arrived unannounced and unmarked, and that it handed back the first 2,000 lines; that was false in both halves and is corrected here. (Worded so this paragraph does not itself match the detector below.) **Re-measure rather than trusting this paragraph** — `docs/planning/read-cap-premise-correction-plan.md` Appendix A is the only instrument that can falsify it, since nothing in this repository can invoke the agent's `Read` tool. This repository's own action ledger did cross the cap, and the overrun was found by accident rather than by any check:
...
- **A remedy proportionate to the finding** (issue [#67](https://github.com/KJ5HST/methodology/issues/67)) — the stale-copy warning answered "this one copy is old" with a bare `--sync`, which is scoped from the canonical's own location and rewrites **26 files across 25 repos**. It now leads with a project-scoped remedy — `--sync [DIR]`, a new single-target scope for the existing sync mechanism — and offers the portfolio-wide path only alongside `--dry-run`. Separately, bare `--dry-run` used to fall through to a full scan and *write* two files; it is now an error. **`--sync`'s blast radius itself is unchanged** — the tool merely stops recommending it undirected.
