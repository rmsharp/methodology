# CHANGELOG.md — archive: 2026-08-12 → 2026-09-02
...
Holds **54 record(s), 2026-08-12 → 2026-09-02**. Cut key: `2026-09-02`. Counts here are computed from the file
...
S133 `next_steps` (2), operator-confirmed at the Phase 0 gate. Readiness items 3/4/9/10
(`docs/planning/upstream-read-set-pr-plan.md` §11.3) are operator decisions and were offered and
declined for this session.

**Re-derived at claim rather than inherited — all 19 proofs run in this tree:** 5 FAIL, matching
S133's count exactly. **Class A** — `CHANGELOG-through-2026-08-02`, `-08-09`,
`HANDOFFS-through-2026-08-02`, `-08-09`, every one generated by `v1.1.1`, failing L1/L3 on record
content. That class was **already answered by S88**: archives intact, fault entirely in the proofs,
root cause `injected = 1 if trims_the_ledger else 0` being a 0/1 flag rather than a count.
**Class B is the open one** — `HANDOFFS-through-2026-08-25.md.verify.sh`, generator **`v1.3.0`**,
failing **L2 only**, while five sibling `v1.3.0` proofs pass.

**The question, stated so it can be answered wrong:** is that L2 failure benign front-matter
maintenance (S133's hypothesis — S132's `ec87d08` rewrote the shard-count line the proof pinned),
or is it evidence of something the front-matter story does not cover?

**BL-36's binding constraint is obeyed:** no `.verify.sh` is regenerated in this session. A
regenerated proof over lost content would pass and would destroy the only evidence that anything is
wrong. The deliverable is an adjudication — one audit report — not a repair.

**Phase 0 also found `HANDOFFS.md` in breach of its own ratified retention policy**: 5 receipts
against a cap of 4, front matter asserting 4, and the file at 58,592 B past the 56,750 B one-read
cap it cites. Reported, not fixed — a trim generates a 20th `.verify.sh` into the very artifact
class under adjudication here, so it is sequenced after, not before. This claim adds a 6th receipt
and makes the breach one worse; that is a known cost, recorded rather than hidden.
