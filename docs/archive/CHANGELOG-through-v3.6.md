# Changelog — Authoritative Action Ledger, archive: v3.0 → v3.6
...
file** — this is the same ledger, continued, not a different document.

Holds **50 entries, 2026-06-25 → 2026-07-26**: every action from the v3.0 relicense through the
dashboard signal-integrity campaign, up to and including the work that shipped as **v3.6**. The live
...
(`git log -1 --format=%H -- CHANGELOG.md`), and the dashboard's `_find_action_ledger` resolves
`<root>/CHANGELOG.md` and nothing else. The one consumer that loses coverage is `bin/model-report`,
whose Source 1 parses a single ledger file — point it here when you want the full history:

```
bin/model-report --changelog docs/archive/CHANGELOG-through-v3.6.md
```

As of the split all **5** `**Model:**` bullets are in the live file, so Source 1 loses nothing today.

**Why this directory.** `docs/archive/` is a subdirectory, so it is structurally invisible to the
dashboard's `_find_changelog`, which scans only the project root and `docs/` and skips non-files.
That matters: v3.6 Layer 2 fixed a real bug where a root `CHANGELOG-archive.md` sorted *ahead* of
`CHANGELOG.md` (`-` is 0x2D, `.` is 0x2E) and freshness was measured against a deliberately frozen
archive. Keeping archives out of the scanned bases means that class of shadowing cannot recur here.

**One mechanical edit to the moved text, and only one.** These entries were written at the repo root,
where a root-relative link to `CLAUDE.md#versioning` resolves; from `docs/archive/` it does not. All
**11** occurrences — the only relative links anywhere in the moved payload, and every one of them the
identical string — were given a `../../` prefix. The referent is unchanged and no other byte moved:
reversing that single substitution reproduces the removed payload exactly, md5
`f3d20156fae054d59b0b0447e3b55b06`. No dated entry was otherwise rewritten (the v2.7.1 convention).
...
- **Commits:** `307a7a8` (plan corrections + Learning #12) + `081d77c` (v3.6 narration + the
  mirror-clause completeness fix) + `79fb2b1` (boundary-review fixes to this session's own
  corrections) + this commit (adopter-facing scoping of the doc-only guarantee, the `CLAUDE.md`
  §Tools row, and the churn-figure correction) + the close-out receipt commit (fork `main`;
