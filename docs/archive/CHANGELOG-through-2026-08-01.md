# Changelog — Authoritative Action Ledger, archive: 2026-07-27 → 2026-08-01

Retired entries from the root [`CHANGELOG.md`](../../CHANGELOG.md), moved here so the live ledger
stays small enough to read in one pass. **Newest on top, `## YYYY-MM` sections, exactly like the live
file** — this is the same ledger, continued, not a different document. The span immediately older
than this one is [`CHANGELOG-through-v3.6.md`](CHANGELOG-through-v3.6.md).

Holds **18 entries, 2026-07-27 → 2026-08-01** — from the v3.6 release through the fork resync with
`upstream/main`. The live file resumes at the 2026-08-02 entry *"`bin/check-handoff` learned the
Phase 1B stub schema"*. Counts here are stated once and computed from the file itself:

```
grep -c '^### ' docs/archive/CHANGELOG-through-2026-08-01.md          # 18
```

**The split key is a DAY — a labelled departure, not a new default.** BL-9 L2 settled the file-split
axis as the **release frontier**, because a shipped release is a cut nothing can ever be written back
into. *That reasoning stands; this split does not overturn it.* **The release axis was unavailable,
not rejected:** v3.6 is the previous shard's own boundary and no release has shipped since, so no
frontier existed at or below the cap. Manufacturing one is an outward-facing act on a paused upstream
channel.

The next calendar seam was *measured before it was chosen*, and it failed. Archiving `## 2026-07`
would have moved 7 entries / 237 lines, leaving 994 lines — **21.1 entries of headroom, re-firing the
trigger inside 2–3 sessions.** That is exactly the outcome BL-9 L2 predicted for calendar cuts ("the
calendar does not partition this ledger"), reproduced by measurement rather than inherited from the
sentence. The day seam moves 18 entries / 726 lines, and it agreed to within one entry with the
successor trigger's own reset arithmetic derived independently — **that agreement, not either figure
alone, is the basis.**

**A day is frozen by convention; a release is frozen by construction.** Nothing structurally prevents
an entry dated 2026-08-01 from being written after this cut — only prepend-only practice does. **If a
release frontier is available at the next archive, prefer it.** The governing trigger is stated once,
in the live file's front matter, and is deliberately not restated here: the previous shard restated
it and the restatement went stale (it still names a `~1,200` line level and a `Released vX.Y` cut,
neither of which governs any more).

**Nothing reads this file automatically, and that is deliberate** — re-verified against this repo at
the split rather than copied forward. The FM #27 pre-commit gate matches the literal staged path
(`grep -qxF 'CHANGELOG.md'`, `.githooks/pre-commit:39`), Phase 0 reconcile is frontier-based
(`git log -1 --format=%H -- CHANGELOG.md`), the dashboard's `_find_action_ledger` resolves the root
file only, and `_find_changelog` scans just `(<root>, <root>/docs)` — `docs/archive/` is not a base,
so no archive can shadow the live ledger by sort order. The one consumer that loses coverage is
`bin/model-report`, whose Source 1 parses a single ledger file; point it here for this span:

```
bin/model-report --changelog docs/archive/CHANGELOG-through-2026-08-01.md
```

Pointing it here is **not optional** for this span. Source 1's regex matches only the seed's list form
`- **Model:**` (`bin/model-report:51`), and all **5** bullets in the corpus written that way are in
this file; the live ledger's **9** use a bare `**Model:**` form the tool cannot parse, a drift that
began at `1298af7` (2026-08-02). So after this split `bin/model-report` with no arguments reports an
*empty* Source 1 — not because the split hid anything, but because the only entries it could ever
read are the ones that moved here. Raised as **BL-20**. Count both dialects; one literal is a sample,
not a population:

```
grep -cE '^-?[[:space:]]*\*\*Model:\*\*' docs/archive/CHANGELOG-through-2026-08-01.md   # 5
```

**One mechanical edit to the moved text, and only one.** These entries were written at the repo root,
where root-relative links resolve; from `docs/archive/` they do not. All **7** occurrences across
**5** distinct targets — `CLAUDE.md#versioning` (×3), `docs/RELEASE_HISTORY.md`,
`docs/planning/model-use-provenance-plan.md`,
`docs/planning/changelog-authoritative-ledger-gate-plan.md` and
`docs/archive/CHANGELOG-through-v3.6.md` — were given a uniform `../../` prefix. *Uniform* is what
makes it provable rather than merely careful: replacing `](../../` with `](` throughout reproduces
the removed payload byte-for-byte, md5 `583934b721c13425fe8fcbdda9571eec`. This differs from the
previous shard, where all 11 relative links were one identical string; here five distinct targets
share one rule, so the rule — not a string — is what must be inverted. No dated entry was otherwise
rewritten (the v2.7.1 convention).
