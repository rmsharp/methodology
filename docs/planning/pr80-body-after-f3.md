**Base `main`, head `read-set-budgets` (`aa36fd8`).** This merges the four PRs already staged on
that branch — #76, #77, #78, #79 — plus your merge of `main` into it (`b82dcff`) and the four commits
that answer your review: F1 (`5c9f0f3`, `d4e1570`), F2 (`37740763`) and F3 (`aa36fd8`).
`git merge-tree` against `main` (`9fa3141`) reports no conflicts. 28 files, +7,892 / −580.
...
|---|---:|---:|---:|
| `main` today | 81,493 | **28,610** | 114.4% — **refused** |
| with this merge | 68,548 | **24,278** | **97.1% — fits, 722 tokens spare** |
...
`main`'s doubled pair reports 57,219 tokens → **28,610**; read undoubled it reports **28,612**, so
the method reproduces itself to 3 tokens. The merge's doubled pair reports 48,555 → **24,278**; this
branch's alone, 47,805 → 23,902 (67,581 B). The 967 B between them is your S16 paragraph in
`SAFEGUARDS.md`, which `main` has and this branch does not — it is in both rows above. At the
conservative 2.27 B/token floor the merged pair still reads over (30,197 tokens); **at every
...
**The root `.context-budget.json` now says the same thing** (your F3). It holds the pair to the
25,000-token read cap in tokens at each file's measured density — 19,200 for `SESSION_RUNNER.md`,
5,800 for `SAFEGUARDS.md` — and no longer budgets the two ledgers, which are read in part and
bounded by the trimmer's archive trigger. `python3 starter-kit/context_budget.py --status` exits
**0** on this branch and on its merge into `main`, where it exited 2.
...
14 of 27 manifest rows change. The corpus grows **659,755 → 839,383 B (+179,628)** — stated first
...
| `read-set-budgets` (`37740763`), after F2 | **115 passed / 1 failed** | 1 |
| `read-set-budgets` (`aa36fd8`), after F3 | **115 passed / 1 failed** | 1 |

The F2 and F3 rows ran in `--no-local` clones and move nothing: 116 rows at `d4e1570`, `37740763` and
`aa36fd8`, not one differing even in a number.
