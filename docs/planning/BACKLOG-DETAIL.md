...
**BL-41 — `methodology_trim.py` cannot trim `HANDOFFS.md` at all right now: its shard-naming scheme
and `bin/tests.sh` Test 34's retention floor have no common solution. Raised 2026-08-18 (S99),
found, not fixed.**
*Blocks the routine remedy for the FM #28 ceiling on `HANDOFFS.md`, and will keep blocking it while
the file grows.*

A shard is named `HANDOFFS-through-<date of the NEWEST ARCHIVED record>.md`, and the tool refuses to
overwrite an existing shard — correctly: *"a collision destroys the earlier shard's records while
all three assertions still pass, because none of them quantifies over any other file in
`docs/archive`."* Retention, meanwhile, is by COUNT, and Test 34 floors it at three receipts.

At S99's close-out the ledger held S96 (2026-08-17), S97 (2026-08-17), S98 (2026-08-18),
S99 (2026-08-18), and `docs/archive/HANDOFFS-through-2026-08-17.md` already existed from S98's trim:

```
--cut 3   archives S96          -> HANDOFFS-through-2026-08-17.md   TAKEN, refused
--cut 2   archives S96, S97     -> HANDOFFS-through-2026-08-17.md   TAKEN, refused
--cut 1   archives S96,S97,S98  -> HANDOFFS-through-2026-08-18.md   free, but retains ONE
```

Any cut that archives S96 without also archiving S98 produces the taken name; any cut that reaches
S98 drops below the floor. **The tool's advice — "Disambiguate with `--cut`" — has no solution here,
because every admissible cut yields the same name.** Verified by running all three cuts as dry runs
against the live ledger (S99); no `--write` was issued.

**It resolves by itself, expensively.** Once enough receipts accumulate that a floor-respecting cut
spans past 2026-08-18 — six receipts, so roughly two more sessions — `--cut 3` archives S96/S97/S98
and the name is free. The cost of waiting is the ledger sitting over its ceiling at ~18 KB per
receipt while it happens, which is the very condition the trim exists to prevent.

**Three candidate fixes, none chosen, none authorised.** (1) Disambiguate the shard name when the
derived one exists (`-2`, or the record range rather than the end date) — a change to a DISTRIBUTED
file, so it needs a go-ahead. (2) Name the shard from the RECORD RANGE (`S96-S97`) rather than a
date, which also fixes the `CUT_STRADDLES_DAY` advisory the tool already prints on the same runs.
(3) Accept the overshoot and drop the Test 34 floor to one, using the stated-SKIP machinery S96 built
(BL-40 (b)) — cheapest, and it discards six invariants, so it is the option to argue against.

**Related:** this is the same SHAPE as the collision S98 found and closed one layer down — two
limits set independently on one artifact, each correct alone, multiplying into a constraint nobody
checked. That one was floor x mean receipt vs ceiling - header. This one is naming-by-date vs
retention-by-count. Learning #35 states the general form.
