...
| **BL-57** | **HIGH PRIORITY** — the `CHANGELOG.md` rules contradict each other across the framework; fix here and in six adopters, aiming at an upstream PR. **Planned at S162, operator-approved** (Q1–Q4 all A): [`changelog-rules-contradictions-plan.md`](changelog-rules-contradictions-plan.md) — **P1 done at S167, P2 at S168, P3 at S172, P4 at S173** on branch `bl57/changelog-rules` (`83a12f0`, all backed up to `origin`; `upstream/main` merged in at `9e1dfeb` and, after #82 merged, at `52ad407`); next is P5, fork `main` adopts it — **after a resync of fork `main` with `upstream/main`** (operator, S173). **The resync is planned (S175):** [`upstream-resync-2026-09-plan.md`](upstream-resync-2026-09-plan.md), RATIFIED after S175 (operator, picker: every recommendation), then R1 and R2. **R1 done at S176** (merge `5c2bd59`, plan §7); R2 next | [detail](BACKLOG-DETAIL.md#bl-57) |
