...
| **BL-59** | `HANDOFFS.md` retention — **operator decided N=1 (2026-09-16)**; measured, **N=1 is not reachable today** (Test 38 needs ≥2 live records: 14 failures), **N=2 + the fold is clean** (299/0/6 skips). Remaining for N=1: a frozen fixture for Test 38 (since landed, `d13a165`). Fork-local — the distributed seed names no retention number. **S174: the header cut is done** — the shard index moved to [`docs/HANDOFFS_ARCHIVE_INDEX.md`](../HANDOFFS_ARCHIVE_INDEX.md) and the front matter fell 7,028 → 3,929 B of the 7,168 B reserve, so a trim no longer grows it | [detail](BACKLOG-DETAIL.md#bl-59) |
