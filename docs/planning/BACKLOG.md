...
| **BL-54** | `bin/sync` and `bin/status` walk history without `--full-history`, so a version from a merge's other side reads as locally modified. **Operator, S173:** fix after BL-57's P5 and before P6. **NEXT (operator, S178): P5's merge made it bite.** `methodology_trim.py` now reads locally modified in 3 adopter copies; `--full-history` at `bin/status:56` restored every row on scratch copies (plan item (12)). Its upstream PR is its own go-ahead | [detail](BACKLOG-DETAIL.md#bl-54) |
