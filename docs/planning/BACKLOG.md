...
BL-42, BL-43, BL-44, BL-46, BL-47, BL-48, BL-49, BL-50, BL-51, BL-52, BL-53, BL-54, BL-55, BL-56, BL-57, BL-58, BL-60, BL-61, BL-62.**
...
| **BL-53** | How many learnings before old ones retire — `FRAMEWORK_LEARNINGS.md`'s limit is now only a growth warning (81,920 B since S159). **Operator, S173 (2026-09-16, picker):** **decide the retirement rule inside the fork resync**, which must reconcile fork `main`'s 66 rows with upstream's 13 anyway. **S175:** posed as decision **D1** of [`upstream-resync-2026-09-plan.md`](upstream-resync-2026-09-plan.md) §3 — upstream has since added its own #15 and #16, so the numbers collide; recommended (A), with a trial of it measured. **Operator, after S175 (picker): (A)** — the distributed file becomes upstream's; the fork's rows #15–#66 move verbatim to `docs/FORK_LEARNINGS.md`, carried out in R1. **S176: done** (`5c2bd59`, `8cfaf0d`). The fork file sits under the unchanged 81,920 B warning with 14,743 B of headroom, so the retirement question has eased, not been answered | [detail](BACKLOG-DETAIL.md#bl-53) |
| **BL-54** | `bin/sync` and `bin/status` walk history without `--full-history`, so a version from a merge's other side reads as locally modified. **Operator, S173 (2026-09-16, picker):** **fix after BL-57's P5 and before P6**, when the adopters first sync from the resynced fork `main`; its upstream PR is its own go-ahead | [detail](BACKLOG-DETAIL.md#bl-54) |
...
| **BL-57** | **HIGH PRIORITY** — the `CHANGELOG.md` rules contradict each other across the framework; fix here and in six adopters, aiming at an upstream PR. **Planned at S162, operator-approved** (Q1–Q4 all A): [`changelog-rules-contradictions-plan.md`](changelog-rules-contradictions-plan.md) — **P1 done at S167, P2 at S168, P3 at S172, P4 at S173** on branch `bl57/changelog-rules` (`83a12f0`, all backed up to `origin`; `upstream/main` merged in at `9e1dfeb` and, after #82 merged, at `52ad407`); next is P5, fork `main` adopts it — **after a resync of fork `main` with `upstream/main`** (operator, S173). **The resync is planned (S175):** [`upstream-resync-2026-09-plan.md`](upstream-resync-2026-09-plan.md), RATIFIED after S175 (operator, picker: every recommendation), then R1 and R2. **R1 done at S176** (merge `5c2bd59`, plan §7); **R2 done at S177** (merges `421ebf9`, `7245f79`, `d4ac950`; D2 `e55f204`, D3 `e1b6bdf`; plan §8): fork `main` contains `upstream/main` `6b29d3d`. **P5 next**: merging `bl57/changelog-rules` into `main` conflicts in 5 files (computed at S177, plan §8). **Operator, S177:** no `CHANGELOG.md` trim at its trigger, per the 2026-09-14 decision. **P12's PR also carries BL-62** | [detail](BACKLOG-DETAIL.md#bl-57) |
...
## Completed items (BL-1 – BL-7, BL-9, BL-10, BL-45, BL-59)
