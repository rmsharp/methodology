...
pushed (16 ahead of `origin/master` `4cfe2dad` in all, with S717's backfill and S718's four)~~ **pushed 2026-09-19 after
S194's close-out, a fast-forward `4cfe2dad..4565c39d` of 16 commits, with S717's backfill and S718's four (read back at
S195: `origin/master` = `4565c39d`, also that project's HEAD; recorded here at S195).** Every per-push workflow passed on
`4565c39d`: lint, test-coverage, pkgdown and R-CMD-check (run `35466135548`, finished 20:34:58Z), the first full
`R CMD check` of P10's tree. `shinytest2` runs nightly or on dispatch, not per push, and has not run on it. The eleven:
`74243f04` (Phase 0 backfill of `312996b0`), `d064993a` (claim),
...
**P11 decided at S195 (2026-09-19, operator, picker); not yet run.** Measured read-only on `model_project_constructor`
at `a18706f`, every option run first in a `--no-local` scratch clone: **(a)** the runner's task-to-workstream rows and
its *Wiki sync* paragraph (with the live path) move into `CLAUDE.md`; its step 5, *"run the external dashboard, never a
copy here"*, is retired, since the sync installs `methodology_dashboard.py` at the root and every later sync rewrites it.
**(b)** The ledger adopts the rules going forward, cadence included: the 156 legacy entries stay byte-identical, the
pointer and marker replace the header's Keep-a-Changelog lines, and every action from P11 on gets a tagged entry under
`## YYYY-MM` above `## [0.3.0]`, superseding the project's `PROJECT_CONVENTIONS.md` §2 cadence. Launch prompt:
[`bl57-p11-model-project-constructor-launch-prompt.md`](bl57-p11-model-project-constructor-launch-prompt.md). What the
measurement found, for P11 and the upstream route:
(32) **A synced file can read *locally modified* while holding no local edit, because its version came from history
the source no longer has.** `model_project_constructor`'s `SAFEGUARDS.md` is byte-identical to blob `6ba2c156`, from the
fork's merge `b91ac8c` (2026-04-10), which survives only on the pre-rebase branch `backup/pr9-pre-rebase`; `bin/sync`
walks fork `main`'s history and cannot see it, so it refuses the file. The runner is the same commit's blob `0c4c65af`
plus the project's three edits (22 lines differ; S192's 29 were measured against `7073dec`, which is not on fork `main`
either). So one of the two refused files held nothing to preserve. Before moving a *locally modified* file's edits, diff
it against every blob of that path on every ref (`git log --all --full-history -- <path>`). Not checked for
`feedback-loop-comparison`, the other project refused at S191 (item (27), decision (c)).
(33) **A project's own conventions can set the ledger's cadence, and the P11 row posed only its format.** Row (b) asked
tagged or legacy. `PROJECT_CONVENTIONS.md` §2 (`:17`–`:49` at `a18706f`) gated entries on shipped-code changes under two
rulings marked SETTLED (2026-08-17 and 2026-08-25), and the runner the project carried predated the ledger rules, so
nothing had to override. The synced runner's Phase 0 step 6 would backfill every commit without an entry: 87 of the last
