...
- Zero of the **22** distributed `.md` files state any ledger size, archive, split, or truncation
  policy. (DISTRIBUTION is 22 `.md` + **2** `.py` as of S39'; it was 22 + 1 at S34 and 21 + 1 when this was measured.) The keyword hits are an example app's CRUD
...
shipped. An adopter receives **two** executables, both from `starter-kit/`: the dashboard, and
`methodology_trim.py` since S39'.
...
**D4 — Three verified defects inside the dashboard, one of the two executables adopters receive.**
...
| **S36** | **Fix the three dashboard defects (D4).** Independently valuable and a precondition for trusting any dashboard row: (a) root-date query returns the newest commit, not the oldest; (b) a 2,090-line `.md` cannot trip the large-file risk; (c) the `methodology` self-exclusion | **G2** | — | **SHIPPED (fork) 2026-08-03** by fork session **S37** (the two axes swapped places — last session was fork S36 executing queue item S37). All three fixed in both twins, `DASHBOARD_VERSION` **2.10.3 → 2.11.0**. Two premises in this row needed correction: (a)'s consequence was **overstated** — the dead risk was unreachable for every *active* low-commit repo, not "never fired", since a stale repo tripped it for the wrong reason; and (b) **cannot be done as written** — `test_large_file_ext_filter` ratifies the opposite, so it shipped as a *second* risk keyed to silent truncation, labelled as added policy. Review also caught a defect neither this row nor the design anticipated: `discover_projects()` has a **second consumer on a write path**, so (c) alone would have made `--sync` install a third copy into the canonical repo's own root. PR still needs a go-ahead |
...
| **S38** | **Dashboard row per grow-and-must-be-read file** — headroom and whether the trigger has fired. Read-only. Names the trimmer **only when it is present**, with **two tests**: the named command is one the trimmer really accepts, and a no-trimmer fixture still points at the documented manual procedure | **G2** | S36, S37 | **SHIPPED (fork) 2026-08-03** by fork session **S38** — the two axes agree this session, having swapped twice. `collect_trim_metrics` in both twins, **37 new tests**, `DASHBOARD_VERSION` **2.11.0 → 2.12.0**, producer-mutation harness **31 killed / 0 survived**. **Three premises in this row needed correction and two changed the build.** (a) *"Read-only"* and §1.3's *"reads the number rather than re-deriving it"* are in tension with §1.3's own **owed agreement test**: a number obtained by parsing `--check` makes that test an identity that cannot fail (Learning #16). The dashboard therefore **computes** the line metric and reads only the trimmer's **budget constant** by regex — §7.1's stated precedent, and the test can now genuinely go red. (b) The absent branch **cannot** *"point at the documented manual procedure"* — **no distributed file documents ledger archiving**, which is what S40 is for; §11 Phase 5 says so in the same document. It states the measurement and discloses the unrun byte metric instead, labelled as a departure. (c) The trimmer is canonical-only and lives only under `starter-kit/`, so the root-anchored probe missed **everywhere** and the *present* branch would have shipped unrun — a `role == "framework"` fallback (added policy) makes it observable on the one repo whose trigger fires. **A 5-lens adversarial review over the uncommitted diff filed 24 findings, 13 survived refutation — all fixed**, plus three divergences found by reading the two implementations side by side. The worst was mine and it was a REGRESSION IN PRE-EXISTING CODE: the new fence regex was named `_FENCE_RE`, silently rebinding a module global of that name, so `_strip_fenced_blocks` stopped stripping *indented* fences and phantom `- [x]` done-marks appeared in the backlog signal (0 → 2 on a fixture). The rest were dashboard/trimmer divergences the agreement test could not see because **every archive shard in this repo happens to shrink its ledger** — a missing `pre <= post` filter (measured at headroom 248 vs `--check`'s 35), a fence closer ignoring the empty-info rule (2 records vs 1), a missing zone-refusal abstention, and locale-dependent blob decoding (20 vs 34 under `LC_ALL=en_US.ISO8859-1`). **Adopter impact, stated:** every adopter gains one `low` row disclosing that neither half of the trigger could measure its ledgers, until S39′ ships the trimmer and S40 writes the doctrine; on a repo with no other risks that moves the displayed worst risk from `healthy` to `low`. PR still needs a go-ahead |
| **S39** | **Decide whether the trimmer ships to adopters**, on S37's evidence. Adopters received exactly one executable when this was queued, so this read as "extend the manifest" vs "stay canonical-only" — and it must be decided *before* S40, because "run this" and "here is the manual procedure" are different documents | **G2/G3** | S37 | **DECIDED 2026-08-03 by the operator: IT SHIPS.** Taken *ahead of its slot* — this row sequenced it on S37's evidence; the operator decided earlier, which is his to make. Recorded at S35, which folded it in as a binding design input (`ledger-trimmer-design.md` §1.4, §6). The remaining work is the manifest/scanner surface, now queued as **S39′**; the PR still needs a go-ahead |
| **S39′** | **Ship the trimmer** — the execution of the decision above, not the decision. Manifest entry, `FRAMEWORK_INSTALLED_SOURCE`, `is_framework_installed` recognition (else a >200-LOC tool at an adopter root flips a doc-only project to "code"), `DASHBOARD_VERSION` bump in both twins, checklist entry, exec-bit assertion. **Measured at 21 files for a simpler precedent, not "one line"** — inventory in [`ledger-trimmer-design.md`](ledger-trimmer-design.md) §6.2 | **G2/G3** | S37, S38 | **SHIPPED (fork) 2026-08-04** by fork session **S39** — `bin/_manifest.py` 23 → 24 dests, `DASHBOARD_VERSION` **2.12.0 → 2.13.0**, both twins. **Three premises in this row needed correction, and one changes what the row was for.** (a) *"`FRAMEWORK_INSTALLED_SOURCE`"* as a task is **inert on its own** — measured, with `methodology_trim.py` on the tuple and no content rule for it, the adopter fixture still read `doc_only` False, a HIGH "No test infrastructure", and a `source_loc` equal to the whole trimmer (1,632 lines at the claim; it is longer now, which is exactly why the figure is dated rather than quoted flat). `is_framework_installed` content-verifies per name, the trimmer declares `TRIM_VERSION` not `DASHBOARD_VERSION`, and carries **zero** of the five structural signatures. The membership list is therefore now **derived from** a per-name content table, so a name cannot reach the exclusion without declaring how it identifies itself — the cheap green edit is no longer expressible. (b) The parenthetical *"else a >200-LOC tool … flips a doc-only project to code"* is right about the danger and wrong about the mechanism, which matters because it is why **BL-22 is not on this item's critical path**: once recognition lands the file is classified `vendor` and `DOC_ONLY_SOURCE_LOC_MAX` never sees it at all. BL-22 stays open on its own merits. (c) *"`DASHBOARD_VERSION` bump | test red"* (design §6.2) invites the reading that a bump is forced. Measured, **nothing forces it** — the full suite is green at 2.12.0 with every functional change in place; the bump is a deliberate choice because adopter-visible behaviour changed, and it moves **five** sites, not the four a `grep '2\.12\.0'` finds (one literal spells the digits with backslashes). **Adopter impact, measured on two real `bin/sync` runs rather than reasoned about:** `source_loc` **0** both before and after; the executables move from `source` into `vendor`, 1 file → **2 files**. The absolute vendor LOC is deliberately not quoted: the first draft of this row published 5,603, and it was false before the session ended because both files kept growing under the very edits that produced the row. Re-derive it — `python3 -c "import sys;sys.path.insert(0,'tools');import methodology_dashboard as md;from pathlib import Path;print(md.collect_all(Path('<synced-repo>'))['files']['by_category']['vendor'])"`, health **47/100 unchanged**, and the fleet-wide `low` *"watched but unmeasured … no `methodology_trim.py` is installed here"* row **is gone** — S38's predicted clearing, verified on a real install. `find_trim_tool` now resolves the **root** candidate, so S38's `role == "framework"` fallback covers exactly one case: the framework repo scanning itself. PR still needs a go-ahead |
| **S40** | **The ledger doctrine** into `starter-kit/HANDOFFS.md` and `starter-kit/CHANGELOG.md`: a stated size norm, the archive trigger as a *rate*, and the one-line-pointer shard convention — the instructions for the cases automation cannot reach | **G3** | S39 | **SHIPPED (fork) 2026-08-04** by fork session **S40** — the two axes agree this session. Both seeds gain a **Size, and when to archive** section; `+8,252 B` across the two, **all of it in the pinned front-matter zone the trimmer never touches**, which is the first change in this campaign that *increases* per-session cost and is stated rather than buried. **Four premises in this row needed correction and two changed the artifact.** (a) *"the archive trigger as a **rate**"* is **half the rule**. `ledger-trimmer-design.md` §5.2 says the byte metric is a **level with hysteresis, not a rate**, and measured at this session's claim the *line* rate does **not** fire on `HANDOFFS.md` (headroom 20 vs a threshold of 15) — **only the byte level does.** A rate-only doctrine would have been silent on the one file this whole campaign exists for, reproducing the blindness §5.1 was written to fix. Both seeds state **both** conditions with the correct form for each; the departure is labelled at §11 Phase 5. (b) *"each state … the shard convention"* — the first draft failed *each*: the `HANDOFFS` seed delegated the entire convention to the `CHANGELOG` seed, and both files are **SEED** disposition (written only when absent), so they install independently and one can arrive pointing at a section the other never delivered. Fixed; the receipt seed now states the path shape, the pointer rule and the span-both-by-glob rule in its own words. (c) §5.4's *"S40 documents choosing it"* is a **fourth** done-item this row omits — the 64 KB budget is judgment, and both seeds now say how to calibrate it. (d) **This row's own "G3 — instructions for users" claim is only true of *new* adopters.** SEED means never-overwritten and `SEED_FORMAT_MARKERS` key on the H1 titles this change leaves intact, so `bin/status` reports every existing adopter's ledger as `present`, not `present (stale format)` — **no existing adopter receives the doctrine and nothing tells them so.** Changing that marker flags every adopter at once and is an operator decision, deliberately not taken here. **A 5-lens adversarial review over the uncommitted diff filed findings across all five; 4 survived refutation, all fixed, and 2 were mine and serious.** The worst: the seed told adopters `--write` *"leaves the change staged for you"* — **false**, the trimmer contains no `git add`, prints `WRITTEN (uncommitted — this tool never commits)` and leaves the shard **untracked**; an adopter believing it would `git commit -a` the shortened ledger while the shard never entered history — the exact loss the three losslessness assertions exist to prevent, invited by the doc rather than the tool. Its provenance is `ledger-trimmer-design.md:722`, which still says *"staged-but-uncommitted"* while its own rollback table two lines below proves otherwise. Second: the worked anecdote was **chronologically backwards** — I wrote that the receipt ledger *"carried ~1,200 lines while its archive fired at 997, and nothing noticed"*; the archive (`7a71df0`, 19:15) **predates** the level (`3aee4e3`, 21:35) by 2h20m the same day, and six lines of this repo's own ledger noticed. Rewritten to the supportable and stronger claim: the level **has never once fired**, the file sits under it while running 4.8× over its byte budget, and a level in the wrong unit says *fine* indefinitely | PR, needs go-ahead |
...
- **Metrics → `methodology_dashboard.py`**, the executable adopters already had. Read-only rows.
  (Written when it was the *only* one; the trimmer below is the second, shipped at S39'.)
...
6. **Does the trimmer ship to adopters, or stay canonical-only?** (§5's queue item **S39**, DECIDED:
   it ships, and shipped at S39'. Adopters received exactly one executable when this was written,
   so it read as "extend the manifest" vs "stay canonical-only",
