...
- the pre-commit hook script does run, but the line inside it that decides whether to call the ratchet at all is `[ -f "$top/.quality-gates.json" ]` — a test for the file in the working tree. Delete the file and that test is false, so the tool is never invoked;
- and if it were invoked, it would pass anyway: `precommit()` reads the staged manifest out of the index and returns clean the moment there isn't one ("manifest not in this commit: nothing to ratchet");
...
*Suggested — three places. The first two are both required: fixing either one alone still lets the deletion through, since each is independently sufficient to wave it past.*

1. **In `.githooks/pre-commit`,** widen the condition that decides whether to call the tool. Today it fires only when the manifest is present in the working tree. Have it also fire when the previous commit had one — `git cat-file -e HEAD:.quality-gates.json` — so that a commit which *removes* the manifest still reaches the tool instead of sailing past it.
2. **In `precommit()`,** handle the case that condition now delivers. Before returning clean on a missing staged manifest, check whether `HEAD` has one with gates in it; if it does and the index has none, refuse — "manifest removed", with the reasoning `compare()` already uses when a single gate disappears ("removing a gate is the loosest loosening").
3. **In the dashboard,** change only how a deleted manifest is *represented*, and the code you already have will do the rest. The chain today: the history walk reads the manifest at each commit that touched it; at the commit that deleted it the read fails, so that version is recorded as unreadable; and the comparison loop skips any pair in which either side is unreadable. The deletion is therefore never compared to anything — and, incidentally, neither is the commit on the far side of it.

   Record that version as a manifest with an **empty gate list** instead, and the comparison runs. `_gate_loosenings` works by walking the *older* version's gates and asking, for each, whether a gate of that name still exists in the newer one; if not, it appends `"removed"`. Against an empty gate list, no name is found, so that branch fires once for every gate that existed — which is the correct reading of deleting the file. That is the relationship: an empty gate set is simply the input that makes "every gate is missing" true, and "a gate is missing" is the one case the function already handles. No new detection logic is required; only a different answer to the question *"what was the manifest at that commit?"*
...
- The budget tool judges this file at **2.93 bytes/token** — the seed's fallback, and `.context-budget.json` says outright that the value is inherited rather than measured here. Measured on this branch it is **2.519**. So the tool reports 20,188 tokens where there are 23,482.5 — **14% low**. Since it then compares that figure against the 25,000-token ceiling it derives for this file, it shows roughly **4,800 tokens of headroom where there are about 1,500**.
...
- **Backing out of the ratchet locks the repository.** `find_root()` locates the project by walking up the directory tree looking for `.quality-gates.json` itself. So once the manifest is gone the tool can no longer find the project at all: it prints *"no `.quality-gates.json` found … refuses to invent thresholds"* and exits 3 — before `precommit()` is ever reached, so none of the HEAD-versus-index reasoning in (a) gets a chance to apply. The hook that `install-hook` writes calls the tool with no guard around it and passes that exit code straight on. The result: after someone removes the manifest with `--no-verify`, **every subsequent commit is refused**, unrelated ones included, by a message that names no cause and suggests no remedy. An adopter who tries the ratchet and backs out is left needing `--no-verify` forever. *Suggested:* locate the root the way the chained hook already does, with `git rev-parse --show-toplevel`, and treat a missing manifest as a state to judge rather than a reason to abort — pass when `HEAD` has none either, refuse with (a)'s "removed" message when it does.

- **The dashboard misses two loosenings the tool catches.** It reports a changed threshold only when the direction stayed the same, and it ignores commands entirely. A direction flip (*at least 5* → *at most 5*) is refused by the tool but **reported as nothing**; swapping a gate's command for `echo 'Ran 999 tests'` is warned about by the tool and **reported as nothing**. That matters because the bypass message promises the opposite: *"the manifest's git history shows the loosening and the dashboard reports it as a risk."* A bypassed flip leaves no advisory at all. *Suggested:* in `_gate_loosenings`, report a direction flip as a loosening in its own right — it currently compares two thresholds only after confirming both versions share a direction, so a flip falls through both branches and out of the function. And add a separate, lower-severity advisory when a gate's `command` or `extract` differs between versions, mirroring the warning the tool itself already prints.

- **The coverage bonus rewards a gate's name.** The dashboard awards +2 testing points when a *passing* gate's **name** matches "coverage". A gate named `coverage` whose command is `echo 100` earns it — I checked. The plan explicitly declined "any coverage floor without a faithfulness check beside it", and this rewards exactly that. It also reads it from the results file, which is gitignored — so the number earning the points was written by whoever last ran the tool on their own machine, and no reviewer ever saw it. *Suggested:* drop the bonus until a faithfulness gate is required alongside, and either way score the measurement rather than the name.

- **`install-hook` breaks every commit in a fresh clone of this repository.** The chain runs like this. `install-hook` decides *where* to write by reading `core.hooksPath`. With it set to `.githooks` — the documented setup — it finds the existing hook already there, declines to overwrite it, and prints a line to chain instead; that path is safe. **A fresh clone has `core.hooksPath` unset**, so it falls back to `.git/hooks/pre-commit`, where nothing exists, and writes. What it writes is the *adopter* hook: `exec python3 "$(git rev-parse --show-toplevel)/quality_ratchet.py" --precommit`. In this repository the tool is at `starter-kit/quality_ratchet.py`, not at the root — so that path doesn't resolve. `install-hook` itself exits 0, and every commit after it fails with *"can't open file `<root>/quality_ratchet.py`"*. `.context-budget.json` already carries exactly this warning for the sibling tool ("DO NOT RUN `install-hook` HERE", with the `rm .git/hooks/pre-commit` recovery); `.quality-gates.json` carries none. *Suggested:* add the same warning, or have `install-hook` write the path of the file that is actually running instead of assuming the root.
