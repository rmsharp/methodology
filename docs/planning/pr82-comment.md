...
3. **In the dashboard,** when the manifest can't be parsed at a commit *because that commit deleted it*, treat it as a manifest containing zero gates rather than as unreadable. `_gate_loosenings` already reports every absent gate as "removed" — it simply never receives that input today, so the branch that would catch this is already written and merely unreachable.
