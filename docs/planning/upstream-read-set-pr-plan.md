...
| 3.4(d) | the `KeyError` sites are at `:339` and `:892` | Stale. They were `:450`/`:1025` when S129 opened and are **gone** now, replaced by `class_spec()` | The plan's numbers belong to the file as of commit `9e71f83` (919 lines) — a **commit**, not a blob (`git cat-file -t` says so; S129 wrote "blob" and a reviewer caught it). **Navigate by symbol** (`grep 'def <name>'`), never by this plan's line numbers. |
...
2. **Both sides of such a comparison must be run with the CANONICAL tool, on FROZEN inputs.** All
   ...
   branch. Only `--precommit`, `--selftest`, `--calibrate` and `--help` are write-free — all four are dispatched before `append_history()` is reached. (S129 first wrote this list without `--calibrate`; corrected after a reviewer ran it and diffed the directory.)
