...
7. **Write handoff notes for the next session.** (See below.) You will be judged on these — the next session will score your handoff just as you scored your predecessor's. Write them both to `SESSION_NOTES.md` (the transient scratchpad) and as a durable ` ```handoff ` receipt block in `HANDOFFS.md` — set `status: complete` and fill the six minimum requirements plus the two scores. The receipt is the machine-checkable proof the handoff exists (`bin/check-handoff` asserts its structure, never its quality); a skipped receipt is caught at the next Pre-Flight reconcile.
