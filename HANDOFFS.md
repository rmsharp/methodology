...
active_task: **BL-57: PUT P12's THREE CARRIED FIXES ON `bl57/changelog-rules` (F5, BL-62, BL-63), MERGE THE BRANCH INTO FORK `main`, GATE BOTH; P12 ITSELF IS NEXT SESSION.** Claimed for P12 (plan `docs/planning/changelog-rules-contradictions-plan.md` §P12); the branch turned out to lack F5, BL-62 and BL-63, so the operator re-scoped after a second picker (plan item (23)): F5 links the design doc's pinned public copy and drops `:33`'s `--no-renames` claim; BL-62 test-first; BL-63 in `starter-kit/BOOTSTRAP.md` only; P12 step 2 will merge. The `HANDOFFS.md` trim ran first (`83099b5`, fold `77fe511`). Nothing upstream.
